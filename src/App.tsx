...
import { ArrowRight, BookOpen, Box, Check, ChevronDown, ChevronUp, Clipboard, Download, ExternalLink, Gamepad2, Github, History, Menu, Music2, Sparkles, Trophy, X } from 'lucide-react'
import { assets, game, releases, resources, worlds, type Release } from './data'
...
function Home() {
  ...
  return <>
    <section className="hero"><div className="hero-bg" style={{ backgroundImage: `url(${assets.titleBackground})` }} aria-hidden="true" /><div className="hero-overlay"><div className="container hero-content"><div className="hero-title-stack"><img src={assets.logo} alt="Happy Mario 9" className="hero-logo" /><img src={assets.title} alt="Happy Mario World Reforged" className="hero-title" /></div><div className="hero-panel"><div className="hero-copy"><div className="status-line"><span className="live-dot" /> {t.status}</div><p>{t.description}</p><div className="hero-actions"><a className="pixel-button primary" href={game.github} target="_blank" rel="noreferrer"><Github size={18} /> {t.download} <ExternalLink size={14} /></a><Link className="pixel-button ghost" to="/worlds"><span className="map-icon">▦</span> {t.viewWorlds} <ArrowRight size={15} /></Link></div></div><div className="hero-hud"><span className="hud-label">WORLD</span><strong>01 - 08</strong><div className="hud-divider" /><span className="hud-label">LEVELS</span><strong>32 / 32 <small>READY</small></strong></div></div></div></div></section>
    ...
    <section className="section feature-section"><div className="container feature-layout"><div className="feature-art"><div className="feature-gallery"><img className="feature-shot feature-shot-main" src={assets.preview('1-1.png')} alt="World 1 gameplay preview" /><img className="feature-shot" src={assets.preview('6-2.png')} alt="World 6 gameplay preview" loading="lazy" /><img className="feature-shot" src={assets.preview('8-4.png')} alt="World 8 gameplay preview" loading="lazy" /></div><div className="art-caption">IN-GAME FOOTAGE / WORLDS 01 · 06 · 08</div></div><div className="feature-copy"><span className="section-kicker">{t.inside}</span><h2>{t.featureTitle.split('\n').map((line, index) => <span key={line}>{index > 0 && <br />}{index === 1 ? <span>{line}</span> : line}</span>)}</h2><p>{t.featureDescription}</p><ul className="feature-list">{t.featureItems.map(item => <li key={item.title}><Check size={17} /><span><b>{item.title}</b><small>{item.detail}</small></span></li>)}</ul><Link to="/worlds" className="text-link">{t.exploreWorlds} <ArrowRight size={16} /></Link></div></div></section>
    <section className="section makers-section"><div className="container makers-grid"><div><span className="section-kicker">{t.madeBy}</span><h2>{t.makersTitle}</h2><p>{t.makersDescription}</p></div><div className="maker-sign"><img className="avatar" src={assets.happymario9} alt="Happymario9" /><div><strong>{t.creatorName}</strong><span>{t.creatorRole}</span></div><img className="avatar" src={assets.greenSweet} alt="GreenSweet" /><div><strong>{t.testerName}</strong><span>{t.testerRole}</span></div></div></div></section>
...
function Worlds() {
  ...
  return <><PageIntro eyebrow={t.eyebrow} title={t.title} description={t.description} icon={<Box size={18} />} /><section className="section worlds-section"><div className="container"><div className="worlds-toolbar"><div><span className="section-kicker">{t.select}</span><h2>{t.title.split(',')[0]}</h2></div><span className="completion-badge"><Check size={14} /> {t.completed}</span></div><div className="world-grid">{worlds.map((world, index) => <button key={world.number} className={`world-card ${world.color} ${selected === world.number ? 'selected' : ''}`} onClick={() => setSelected(world.number)}><div className="world-card-top"><span className="world-number">0{world.number}</span><span className="world-status">{selected === world.number ? t.selected : t.ready}</span></div><div className="world-thumb"><img src={world.preview} alt={`${world.name} preview`} loading="lazy" /></div><div className="world-card-bottom"><div><strong>{world.name}</strong><span>{t.items[index].theme}</span></div><div className="level-count">{world.levels}<small> {t.levels}</small></div></div><p>{t.items[index].note}</p></button>)}</div><div className="selected-world"><div className="selected-marker">WORLD 0{selected}</div><div><span className="section-kicker">{t.current}</span><h3>{t.items[selected - 1].theme}</h3><p>{t.currentDescription.replace('{levels}', String(worlds[selected - 1].levels))}</p><div className="selected-gallery">{worlds[selected - 1].gallery.map((image, index) => <img src={image} alt={`World ${selected}-${index + 1} preview`} key={image} loading="lazy" />)}</div></div><span className="selected-arrow"><ArrowRight size={22} /></span></div></div></section><section className="section extra-section"><div className="container extra-layout"><div><span className="section-kicker">{t.bonus}</span><h2>{t.bonusTitle}</h2><p>{t.bonusDescription.replace('<b>', '').replace('</b>', '')}</p></div><div className="bonus-gallery"><img src={assets.preview('ex-1.png')} alt="Extra level preview 1" /><img src={assets.preview('ex-2.png')} alt="Extra level preview 2" /></div></div></section></>
}
...
function Downloads() {
  ...
  return <><PageIntro eyebrow={t.eyebrow} title={t.title} description={t.description} icon={<Download size={18} />} /><section className="section download-section"><div className="container download-layout"><div className="download-main"><div className="download-flag"><span className="live-dot" /> {t.latestBuild}</div><h2>Happy Mario<br /><em>World Reforged</em></h2><p className="download-version">V0.8.0 <span>·</span> 2026.07.22</p><p>{catalog.home.update}</p><a className="pixel-button primary large" href={game.github} target="_blank" rel="noreferrer"><Github size={19} /> {t.github} <ExternalLink size={14} /></a><div className="channel-backup"><span className="channel-backup-label">{t.backup}</span><a className="pixel-button ghost" href={game.download} target="_blank" rel="noreferrer"><Download size={16} /> {t.download} <ExternalLink size={12} /></a><div className="code-row"><span>{t.code}</span><strong>{game.downloadCode}</strong><CopyCode code={game.downloadCode} /></div></div></div><div className="download-poster"><div className="poster-top">HMWR <span>0.8.0</span></div><img className="poster-gameplay" src={assets.preview('8-1.png')} alt="World 8 gameplay preview" /><div className="poster-bottom"><span>8 WORLDS</span><span>32 LEVELS</span></div></div></div></section><section className="section resources-section"><div className="container"><div className="section-heading compact"><div><span className="section-kicker">{t.resources}</span><h2>{t.resourcesTitle}</h2></div><p>{t.resourcesDescription}</p></div><div className="resource-grid">{resources.slice(1).map((resource, index) => <a className={`resource-card ${resource.kind}`} href={resource.href} target="_blank" rel="noreferrer" key={resource.label}><span className="resource-icon">{resource.kind === 'music' ? <Music2 /> : <BookOpen />}</span><span><b>{catalog.resources[index + 1].label}</b><small>{catalog.resources[index + 1].detail}</small></span><ExternalLink size={16} /></a>)}</div></div></section><section className="section history-downloads"><div className="container"><div className="section-heading compact"><div><span className="section-kicker">{t.archive}</span><h2>{t.archiveTitle}</h2></div><Link to="/releases" className="text-link">{t.viewChangelog} <ArrowRight size={16} /></Link></div><div className="archive-list">{releases.slice(1, 5).map((release, index) => <div className="archive-row" key={release.version}><span className="archive-version">{release.version}</span><span>{release.date}</span><span className="archive-add">{catalog.releases.items[index + 1].additions[0] || catalog.releases.items[index + 1].adjustments[0]}</span>{release.href && <a href={release.href} target="_blank" rel="noreferrer" aria-label={t.downloadVersion.replace('{version}', release.version)}><Download size={16} /></a>}</div>)}</div></div></section></>
}
