...
function Home() {
  ...
  return <>
    <section className="hero"><div className="hero-bg" style={{ backgroundImage: `url(${assets.titleBackground})` }} aria-hidden="true" /><div className="hero-overlay"><div className="container hero-content"><div className="hero-title-stack"><img src={assets.logo} alt="Happy Mario 9" className="hero-logo" /><img src={assets.title} alt="Happy Mario World Reforged" className="hero-title" /></div><div className="hero-panel"><div className="hero-copy"><div className="status-line"><span className="live-dot" /> {t.status}</div><p>{t.description}</p><div className="hero-actions"><a className="pixel-button primary" href={game.download} target="_blank" rel="noreferrer"><Download size={18} /> {t.download} <ExternalLink size={14} /></a><Link className="pixel-button ghost" to="/worlds"><span className="map-icon">▦</span> {t.viewWorlds} <ArrowRight size={15} /></Link></div></div><div className="hero-hud"><span className="hud-label">WORLD</span><strong>01 - 08</strong><div className="hud-divider" /><span className="hud-label">LEVELS</span><strong>32 / 32 <small>READY</small></strong></div></div></div></div></section>
